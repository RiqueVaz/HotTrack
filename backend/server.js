                        ...
                        if (btnUrl.includes('/oferta/')) {
                            try {
                                ...
                                if (checkoutId && variables.click_id) {
                                    logger.debug(`${logPrefix} [Checkout Auto-PIX] Detectado checkout ${checkoutId} no botão. Verificando se deve gerar PIX...`);
                                    ...
                                    if (click) {
                                        ...
                                        if (!click.checkout_id) {
                                            ...
                                            if (existingPendingPix) {
                                                ...
                                                logger.debug(`${logPrefix} [Checkout Auto-PIX] PIX pendente já existe (${existingPendingPix.provider_transaction_id}). Usando existente.`);
                                                ...
                                                logger.debug(`${logPrefix} [Checkout Auto-PIX] Gerando PIX automático para checkout ${checkoutId}...`);
                                                ...
                                                if (hostedCheckout && hostedCheckout.seller_id === sellerId) {
                                                    try {
                                                        ...
                                                        if (seller && seller.api_key) {
                                                            ...
                                                            logger.debug(`${logPrefix} [Checkout Auto-PIX] PIX gerado com sucesso. Transaction ID: ${pixResult.transaction_id}`);
                                                        ...
                                                        logger.error(`${logPrefix} [Checkout Auto-PIX] Erro ao gerar PIX automático:`, pixError.message);
                                                        // Não falhar, continuar enviando mensagem
                                                    }
                                                } else {
                                                    logger.warn(`${logPrefix} [Checkout Auto-PIX] Checkout ${checkoutId} não encontrado ou não pertence ao seller ${sellerId}`);
                                                }
                                            }
                                        } else {
                                            logger.debug(`${logPrefix} [Checkout Auto-PIX] Click já tem checkout_id. Usuário provavelmente já acessou checkout. Não gerando PIX automático.`);
                                        }
                                    }
                                }
                            } catch (checkoutError) {
                                logger.error(`${logPrefix} [Checkout Auto-PIX] Erro ao processar checkout no botão:`, checkoutError.message);
                                // Não falhar, continuar enviando mensagem
                            }
                        }
...
app.post('/api/bots/mass-send', authenticateJwt, async (req, res) => {
      ...
      try {
        ...
        if (hasAnyTagFilter) {
            // Construir query base com CTEs para cada tipo de tag (excluindo bloqueados)
            let tagQuery = `
...
app.post('/api/bots/contacts-count', authenticateJwt, async (req, res) => {
    ...
    try {
        ...
        if (hasAnyTagFilter) {
            ...
            // Query simples sem filtro de tags - apenas usuários individuais (chat_id > 0), excluindo bloqueados
            query = `SELECT COUNT(DISTINCT tc.chat_id) FROM telegram_chats tc LEFT JOIN bot_blocks bb ON bb.bot_id = tc.bot_id AND bb.chat_id = tc.chat_id WHERE tc.seller_id = $1 AND tc.bot_id = ANY($2::int[]) AND tc.chat_id > 0 AND bb.chat_id IS NULL`;
            params = [sellerId, botIds];
            
            if (excludeChatIds && Array.isArray(excludeChatIds) && excludeChatIds.length > 0) {
                query += ` AND tc.chat_id NOT IN (${excludeChatIds.map((_, i) => `$${i + 3}`).join(',')})`;
                params.push(...excludeChatIds);
            }
        }
...
async function getContactsByTags(botIds, sellerId, tagIds = null, tagFilterMode = 'include', excludeChatIds = null) {
    // Se não há filtros de tags, retornar todos os contatos (excluindo bloqueados)
    if (!tagIds || !Array.isArray(tagIds) || tagIds.length === 0) {
        let query;
    ...
    // Se não há tags válidas, retornar todos os contatos (excluindo bloqueados)
    if (!hasAnyTagFilter) {
        let query;
        if (excludeChatIds && excludeChatIds.length > 0) {
            query = sqlTx`SELECT DISTINCT ON (tc.chat_id) tc.chat_id, tc.bot_id, tc.first_name, tc.last_name, tc.username, tc.click_id
    ...
    // Construir query com filtro de tags (excluindo bloqueados)
    let tagQuery = `
        WITH base_contacts AS (
            SELECT DISTINCT ON (tc.chat_id) 
                tc.chat_id, tc.first_name, tc.last_name, tc.username, tc.click_id, tc.bot_id
